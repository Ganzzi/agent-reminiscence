    ...
    def _validate_metadata(metadata: Dict) -> List[str]:
        ...
        if 'usage_type' in metadata:
            ...
            if usage_type not in valid_usage_types:
                errors.append(f"Invalid usage_type: {usage_type}. Must be one of {valid_usage_types}")
            ...
            for idx, section in enumerate(template['sections']):
                if isinstance(section, dict):
                    ...
                    if len(desc.strip()) < 10:
                        warnings.append(f"Section {idx} ({section.get('id', 'unknown')}) has very short description")
