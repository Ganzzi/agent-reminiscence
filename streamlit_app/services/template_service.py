...
class TemplateService:
    ...
    def filter_templates(
        ...
        if usage_type:
            filtered = [
                t for t in filtered 
                if t.get('template', {}).get('metadata', {}).get('usage_type') == usage_type
            ]
            
        if priority:
            filtered = [
                t for t in filtered 
                if t.get('template', {}).get('metadata', {}).get('priority') == priority
            ]
    ...
    def get_template_stats(self) -> Dict[str, int]:
        ...
        for templates in all_templates.values():
            for template in templates:
                metadata = template.get('template', {}).get('metadata', {})
